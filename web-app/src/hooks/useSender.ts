...
export function useSender(): UseSenderReturn {
  ...
  const connectionProgressMapRef = useRef<
    Map<
      string,
      { bytesTransferred: number; totalBytes: number; speedBps: number }
    >
  >(new Map());
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      unlistenStart = await listen("transfer-started", () => {
        ...
        connectionProgressMapRef.current.clear();
      ...
      unlistenProgress = await listen("transfer-progress", (event: any) => {
        try {
          ...
          // New format: conn_id:req_id:bytes:total:speed_bps
          if (parts.length === 5) {
            const connectionId = parseInt(parts[0], 10);
            const requestId = parseInt(parts[1], 10);
            const bytesTransferred = parseInt(parts[2], 10);
            const totalBytes = parseInt(parts[3], 10);
            const speedBps = parseFloat(parts[4]);

            // Track progress per connection for multi-receiver scenarios
            const connectionKey = `${connectionId}:${requestId}`;
            if (!connectionProgressMapRef.current) {
              connectionProgressMapRef.current = new Map();
            }
            connectionProgressMapRef.current.set(connectionKey, {
              bytesTransferred,
              totalBytes,
              speedBps,
            });

            // Aggregate progress across all connections
            let aggregatedBytes = 0;
            let aggregatedTotal = 0;
            let aggregatedSpeed = 0;
            connectionProgressMapRef.current.forEach((progress) => {
              aggregatedBytes += progress.bytesTransferred;
              aggregatedTotal += progress.totalBytes;
              aggregatedSpeed += progress.speedBps;
            });

            const percentage =
              aggregatedTotal > 0
                ? Math.min((aggregatedBytes / aggregatedTotal) * 100, 100)
                : 0;

            // Add speed sample and calculate ETA
            speedAveragerRef.current.addSample(aggregatedSpeed);
            const avgSpeed = speedAveragerRef.current.getAverage();
            const bytesRemaining = Math.max(
              aggregatedTotal - aggregatedBytes,
              0,
            );
            const eta = calculateETA(bytesRemaining, avgSpeed);

            latestProgressRef.current = {
              bytesTransferred: aggregatedBytes,
              totalBytes: aggregatedTotal,
              speedBps: aggregatedSpeed,
              percentage,
